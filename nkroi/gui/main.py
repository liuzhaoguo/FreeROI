...
from component.threeDWidget import ThreeDWidget
...
class BpMainWindow(QMainWindow):
    ...
    def _create_actions(self):
        ...
        # 3D view action
        self._actions['3DView'] = QAction(QIcon(os.path.join(
            self._icon_dir, '3DView.png')),
                                          self.tr("3DView"), self)
        self._actions['3DView'].triggered.connect(self._3DView)
        self._actions['3DView'].setEnabled(False)
        #generated by zgf-----------------------------------------------------------------------------------------------
    ...
    def _add_toolbar(self):
        ...
        self._toolbar.addAction(self._actions['3DView'])
    ...
    def _add_template_img(self, source, name=None, header=None, view_min=None, 
        ...
        if self.model.addItem(template_path, None, name, header,
            ...
            self._actions['3DView'].setEnabled(True)
    ...
    def _3DView(self):
        ...
        self.centralWidget().layout().removeWidget(self.image_view)
        self.image_view.set_display_type('3D Volume')
        self.image_view.deleteLater()
        self.image_view = ThreeDWidget(self.model)
        self.centralWidget().layout().addWidget(self.image_view)
    ...
    def _init_data_select(self):
        '''3D or 4D data select'''
        if globavariable.threeD_fourD_flag:
            self.list_view.volume_index_spinbox.setEnabled(True)
            self.list_view.volume_index_spinbox.setRange(0,globavariable.data.shape[3]-1)
            self.list_view.data_select_combo.setCurrentIndex(1)
            self.list_view.data_select_combo.setEnabled(True)
        else:
            self.list_view.volume_index_spinbox.setEnabled(False)
            self.list_view.data_select_combo.setCurrentIndex(0)
            self.list_view.data_select_combo.setEnabled(False)

    def _data_select_combo(self):
        if self.list_view.data_select_combo.currentIndex() == 0:
            self.list_view.volume_index_spinbox.setEnabled(False)
        else:
            self.list_view.volume_index_spinbox.setEnabled(True)


    def _volume_index_changed(self):
        '''volume index changed in 4D data'''
